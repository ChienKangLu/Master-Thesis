...
class exp_dating {
    ...
    async run(){
        ...
        let expSettings:Array<expSetting>=[
            {//0
                DBname:"exp_dating",//exp_attraction_distance_50
                expName:"attraction",//exp_expectation-heuristic-weekTime-reasonable
            ...
            {//1
                DBname:"exp_dating",//exp_attraction_distance_50
                expName:"distance",//exp_distance-heuristic-weekTime-reasonable
                attribution:[Node.index_distance],
            ...
            {//2
                DBname:"exp_dating",//exp_attraction_distance_50
                expName:"attraction+distance",//exp_expectation-heuristic-distance-heuristic-weekTime-reasonable
                attribution:[Node.index_attraction,Node.index_distance],
                attraction_heuristic:true,
                distance_heuristic:true,
                travelTime_heuristic:false,
                edgeExcept_heuristic:false,
                attraction_expect_type:"attraction_expect_square",
                dataStructure:"minheap",
                openlist_comparator:"comparator_heap_normalize_gaussian",
                km15:false,
                nodeLimitNumber:0,//2000
                considerWeekTime:true,
                considerReasonable:true,
        ...
        let minDepth = 6;//2
        let maxDepth = 6;//5
        for(let expSettingIndex =0;expSettingIndex<=2;expSettingIndex++){
            ...
            let end_index:number=0;//random_times-1
            for (let i: number = start_index; i <= end_index; i++) {
                for (let depth: number = minDepth; depth <= maxDepth; depth++) {
                    ...
                    // let start_time: Time = new Time(undefined, 2018, 3, 24, 11, 0, 0);
                    // let time_budget: number = 10;
    ...
    async data() {
        let db=await DBAsync.connectDBAsync("dating");//poitest/finaldata/small_pois/small_pois2
        ...
        coll=await DBAsync.coll(db,"pairwise_all");//pairWise/yilanpairwise/yilanpairwise_all
    ...
    setting(start_id:string,destination_id:string,start_time:Time,
        ...
        if(expSetting.openlist_comparator=="comparator_heap_normalize_gaussian"){
            constraint.comparator_heap_normalize_gaussian=true;
        }
        ...
        if(expSetting.attraction_expect_type == "attraction_expect_square_sign"){
            constraint.attraction_expect_square_sign=true;
        }
